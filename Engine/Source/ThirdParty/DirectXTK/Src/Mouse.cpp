...
void Mouse::ProcessMessage(UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_MOUSEMOVE:
        ...
        {
            RECT windowRect;
            GetWindowRect(pImpl->mWindow, &windowRect);

            int midX = windowRect.left + (windowRect.right - windowRect.left) / 2;
            int midY = windowRect.top + (windowRect.bottom - windowRect.top) / 2;
...
void Mouse::ButtonStateTracker::Update(const Mouse::State& state) noexcept
{
    ...
    relativeScrollWheel = state.scrollWheelValue - lastState.scrollWheelValue;
